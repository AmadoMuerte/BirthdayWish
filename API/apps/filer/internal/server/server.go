...
type Server struct {
	...
	filerService   *service.FilerService
...
func New(runMode *string, cfg *config.Config, storage *storage.Storage, filerService *service.FilerService, log *slog.Logger) *Server {
	server := &Server{
		runMode:      runMode,
		cfg:          cfg,
		storage:      storage,
...
func (s *Server) Start() {
	...
	filerProto.RegisterFilerServiceServer(s.grpcServer, s.filerService)
	...
	go func() {
		s.log.Info("Filer service started",
			"host", s.cfg.App.Host,
			"port", s.cfg.App.Port,
			"mode", *s.runMode)
	...
	select {
	case <-quit:
		s.log.Info("Shutting down filer service...")
		s.Stop()
	case err := <-serverErr:
...
func (s *Server) Stop() {
	...
	s.log.Info("Filer service stopped")
}
